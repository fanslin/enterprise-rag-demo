...
评测集位置：`src/main/resources/eval-cases.json`

每个评测用例包含：

- `id`：稳定用例标识。
- `question`：评测问题。
- `expectedKeywords`：期望召回片段命中的关键词列表。
- `expectedSources`：期望召回来源列表，可以为空。
- `expectedAnswer`：人工参考答案，当前不参与自动评分。
...
1. 根据请求参数解析出 `topK` 和 `similarityThreshold` 的参数矩阵。
2. 对每组参数运行完整评测集。
3. 对每个问题执行检索。
4. 如果没有召回片段，标记为 `NO_RETRIEVED_DOCUMENTS`。
5. 如果召回片段没有命中任一期望关键词，标记为 `MISSING_KEYWORDS`。
6. 如果配置了期望来源但召回来源未命中，标记为 `MISSING_SOURCE`。
7. 否则标记为 `PASSED`。

评测接口返回结构化报告：

- `summary`：跨参数组合的总通过率、通过数、失败数。
- `runs`：每组参数的评测结果。
- `results`：每条用例的通过状态、失败原因和召回引用。
...
这会运行 6 组参数组合，用于观察不同召回数量和相似度阈值对通过率的影响。

这是检索层评测，用来确认知识库导入、Embedding 配置和检索参数是否合理。它还不是完整 RAG 评测，无法覆盖：
